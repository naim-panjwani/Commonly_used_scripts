...
# Syntax: bash merge_bplink1.9_files.sh <dataset1_filename> <dataset2_filename> <output_filename> <temp_identifier>
# Example: bash merge_bplink1.9_files_v2.sh 03_spit_for_sci_ctrls 38_ceu_cts_IIDupdated_18nov2014update 04_UK_SS_merged chr11
...
plink1.9 --noweb --bfile $2 --extract temp_${id}.dataset_snplist.txt  --make-bed --out temp_${id}_dataset2_common_snps
...
plink1.9 --noweb --bfile $1 --extract temp_${id}.dataset2_common_snps.txt --make-bed --out temp_${id}_dataset1_common_snps

echo "5. Attempting 1st merge"
plink1.9 --noweb --bfile temp_${id}_dataset2_common_snps --bmerge temp_${id}_dataset1_common_snps.bed temp_${id}_dataset1_common_snps.bim temp_${id}_dataset1_common_snps.fam --make-bed --out temp_${id}_mergedsets1

if [[ -e "temp_${id}_mergedsets1-merge.missnp" ]]; then
  echo "6. Flip strand at specific identified SNPs"
  plink1.9 --noweb --bfile temp_${id}_dataset1_common_snps --flip temp_${id}_mergedsets1-merge.missnp --make-bed --out temp_${id}_dataset1_flipped_snps

  echo "7. Attempting 2nd merge"
  plink1.9 --noweb --bfile temp_${id}_dataset1_flipped_snps --bmerge temp_${id}_dataset2_common_snps.bed temp_${id}_dataset2_common_snps.bim temp_${id}_dataset2_common_snps.fam --make-bed --out temp_${id}_mergedsets2
  ...
  if [[ -e "temp_${id}_mergedsets2-merge.missnp" ]]; then
    ...
    plink1.9 --noweb --bfile temp_${id}_dataset1_flipped_snps --exclude polymorphic_snp_list_${id}.missnp --make-bed --out temp_${id}_dataset1_polymor_rm
    plink1.9 --noweb --bfile temp_${id}_dataset2_common_snps --exclude polymorphic_snp_list_${id}.missnp --make-bed --out temp_${id}_dataset2_polymor_rm
  
    echo "8. Attempting 3rd merge"
    plink1.9 --noweb --bfile temp_${id}_dataset1_polymor_rm --bmerge temp_${id}_dataset2_polymor_rm.bed temp_${id}_dataset2_polymor_rm.bim temp_${id}_dataset2_polymor_rm.fam --make-bed --out temp_${id}_mergedsets3
