...
if __name__=='__main__':
  ...
  INFO_SCORE_CUTOFF = 0.4

  print(datetime.now().strftime('%c'))
  print('args.inputvcf: ' + args.inputvcf.replace('.gz','').replace('.vcf','')+'.vcf.gz')
  print('args.outvcf: ' + args.outvcf.replace('.gz','').replace('.vcf','')+'.vcf.gz')
  inputvcf = args.inputvcf.replace('.gz','').replace('.vcf','')+'.vcf.gz'
  ...
  vcf = pd.read_csv(inputvcf, skiprows=numHeaderLines, compression='gzip', encoding='utf-8', sep="\t", chunksize=10000)
  print('Extracting dosage')
  writeHeader(inputvcf, outvcf)
  for vcfchunk in tqdm(vcf):
    info_scores = [float(x.split(';')[3].split('=')[1]) for x in list(vcfchunk.iloc[:,7])]
    info_scores_idx = [x > INFO_SCORE_CUTOFF for x in info_scores]
    vcfchunk = vcfchunk.iloc[info_scores_idx,:]
    vcfformatcol = vcfchunk.iloc[:,8].replace('GT:ADS:DS:GP','DS')
    vcfgenos = vcfchunk.iloc[:,9:]
    for i in np.arange(vcfgenos.shape[1]):
      vcfgenos.iloc[:,i] = [x[2] for x in list(vcfgenos.iloc[:,i].str.split(':'))]
