...
getPedCols <- function(chr,geneStart,geneEnd,geneName,bim) {
  ...
  which(bim[,1] == chr & bim[,4] >= geneStart & bim[,4] <= geneEnd)
}
...
for(i in 1:nrow(gene_list)) {
  gene <- gene_list[i,]
  colIndexes <- getPedCols(chr=as.character(gene[,1]), geneStart=as.numeric(gene[,2]), geneEnd=as.numeric(gene[,3]), geneName=as.character(gene[,4]), bim=map)
  ...
  gene_Pvals <- rbind(gene_Pvals, c(Gene=as.character(gene[,4]), P=pval))
}
print("Per-gene permutation tests results:")
print(gene_Pvals)
