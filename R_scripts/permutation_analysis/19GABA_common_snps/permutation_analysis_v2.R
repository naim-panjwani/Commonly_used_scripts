...
pca <- read.table("24_mergedset_pcapc.ped")
if(length(which(is.na(pca[,7])))>0) pca <- pca[-which(is.na(pca[,7])),]

fam <- read.table("11_merged_19GABA_monomorphic_rm.fam", header=F)
...
# Load dataset
map <- read.table("11_merged_19GABA_monomorphic_rm.bim",header=F)
num.individuals<-dim(fam2)[1]
num.snps <- dim(map)[1]
print("Loading raw file")

ped <- read.big.matrix("13_merged_19GABA_monomorphic_rm.raw",
                       type="short",
                       header=T,
                       sep=" ")
# 1. Order the data
...
#write.table(coefs, "26_observed_stats_permutation.txt", quote=F, row.names=F, col.names=T)
