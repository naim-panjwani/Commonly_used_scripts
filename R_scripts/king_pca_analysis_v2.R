...
eigenvalues <- read.table("8_eigenvalues.txt",header=FALSE)
eigenvectors <- read.table("8_mergedset_pcapc.ped")
case_individuals <- read.table("cts_king_list_parents_incl.txt", header=F)[,c(1:2)]
ctrl_individuals <- read.table("SS_randomized_list.txt")[,c(1:2)]

joint <- read.table("../cts_ceu_individuals_relateds_included.txt", header=T, sep=" ")[,c(1:2)]
names(ctrl_individuals) <- c("FID","IID")
joint <- rbind(joint, ctrl_individuals)
#ceu_individuals <- read.table("../ceu_individuals.txt",header=F)

#### SCREEPLOT
png("09_screeplot.png")
...
#eigenvectors <- subset(eigenvectors, as.character(eigenvectors[,2]) %in% as.character(ceu_individuals[,2]))
...
# 2D plots
# Vector 1 vs 2
png("09_2d_pca_plots.png", width=1440,height=1440, pointsize=24)
par(mfrow=c(2,2))
...
write.table(final_caseset, "09_final_case_set_after_pcoutliers_rm.txt", quote=F, row.names=F, col.names=F)
write.table(final_set, "09_final_set_after_pcoutliers_rm.txt", quote=F, row.names=F, col.names=F)
write.table(outliers, "09_outliers.txt",quote=F,row.names=F,col.names=F)
