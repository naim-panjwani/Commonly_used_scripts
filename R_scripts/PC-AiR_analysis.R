...
fam <- read.table(famfilename, header=F, stringsAsFactors=F)
...
#### SCREEPLOT
png(paste0(outfile_prefix, "_screeplot.png"), type="cairo")
plot(eigenvalues$V2, xlim=c(1,10), ylab="Eigenvalue",xlab="Component number", main="Screeplot") #screeplot
dev.off()
...
getFID <- function(IID) {
  ...
  for(i in 1:length(IID)) {
    fid <- c(fid, fam[which(fam[,2] %in% IID[i]),1])
  }
  return(fid)
}
#-------------------
getOutliers <- function(eigen_set, eigen_sd_set, iid_column, eigenvalue_columns) {
...
eigen_cases <- subset(eigenvectors, as.character(eigenvectors$V1) %in% as.character(case_individuals[,2]))
eigen_ctrls <- subset(eigenvectors, as.character(eigenvectors$V1) %in% as.character(ctrl_individuals[,2]))
eigen_joint <- subset(eigenvectors, as.character(eigenvectors$V1) %in% as.character(joint[,2]))

# Outliers from the overall set:
print("Outliers when SD is calculated in cases and controls, within the first 3 PCs:")
(overall_outliers <- getOutliers(eigen_set=eigen_joint, eigen_sd_set=eigenvectors, iid_column=1, eigenvalue_columns=2:4))
print("Outliers when SD is calculated from the controls, within the first 3 PCs:")
(ctrl_sd_outliers <- getOutliers(eigen_set=eigen_joint, eigen_sd_set=eigen_ctrls, iid_column=1, eigenvalue_columns=2:4))
print("Outliers when SD is calculated from the cases, within the first 3 PCs:")
(case_sd_outliers <- getOutliers(eigen_set=eigen_joint, eigen_sd_set=eigen_cases, iid_column=1, eigenvalue_columns=2:4))
print("List of all outliers identified above:")
(outliers <- unique(rbind(overall_outliers,ctrl_sd_outliers,case_sd_outliers)))
eigen_outliers <- subset(eigenvectors, as.character(eigenvectors$V1) %in% as.character(outliers$IID))
...
# 2D plots
# Vector 1 vs 2
png(paste0(outfile_prefix, "_2d_pca_plots.png"), width=1440,height=1440, pointsize=24, type="cairo")
par(mfrow=c(2,2))
with(eigenvectors, plot(V2,V3,type="n", xlab="Vector1", ylab="Vector2"))
with(eigen_ctrls, points(V2,V3, col=colours[2]))
with(eigen_cases, points(V2,V3, col=colours[1]))
# Mark outlier individuals with "green" ring
with(eigen_outliers, points(V2,V3, col="green", cex=1.5))

# Vector 2 vs 3
with(eigenvectors, plot(V3,V4,type="n", xlab="Vector2", ylab="Vector3"))
with(eigen_ctrls, points(V3,V4, col=colours[2]))
with(eigen_cases, points(V3,V4, col=colours[1]))
# Mark outlier individuals with "green" ring
with(eigen_outliers, points(V3,V4, col="green", cex=1.5))


# Vector 1 vs 3
with(eigenvectors, plot(V2,V4,type="n", xlab="Vector1", ylab="Vector3"))
with(eigen_ctrls, points(V2,V4, col=colours[2]))
with(eigen_cases, points(V2,V4, col=colours[1]))
# Mark outlier individuals with "green" ring
with(eigen_outliers, points(V2,V4, col="green", cex=1.5))
...
final_caseset <- eigen_cases[-which(as.character(eigen_cases[,1]) %in% as.character(outliers[,2])),1:2]
final_set <- eigenvectors[-which(as.character(eigenvectors[,1]) %in% as.character(outliers[,2])),1:2]
