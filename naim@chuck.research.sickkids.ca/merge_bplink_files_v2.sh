...
if [[ -e "temp_${id}_mergedsets1.missnp" ]]; then
  echo "6. Flip strand at specific identified SNPs"
  plink --noweb --bfile temp_${id}_dataset1_common_snps --flip temp_${id}_mergedsets1.missnp --make-bed --out temp_${id}_dataset1_flipped_snps

  echo "7. Attempting 2nd final merge"
  plink --bfile temp_${id}_dataset1_flipped_snps --noweb --bmerge temp_${id}_dataset2_common_snps.bed temp_${id}_dataset2_common_snps.bim temp_${id}_dataset2_common_snps.fam --make-bed --out temp_${id}_mergedsets2
  
  if [[ ! -e "temp_${id}_mergedsets2.bed" ]]; then
    echo "Something went wrong; check temp_${id}_mergedsets2.log file"
