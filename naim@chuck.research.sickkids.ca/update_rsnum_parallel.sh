...
# Example: bash update_rsnum_parallel.sh 38_ceu_cts_IIDupdated_18nov2014update.bim snp141.bed.gz  38_new_bimfile 30

bim=$1
num_lines=`wc -l ${bim} |awk '{print $1}'`
num_cores=$4
line_split=`echo $((num_lines / num_cores))`
split -d -a 3 -l ${line_split} $1 tmp.list
outname=$3
i=0
while [ $i -le $num_cores ]; do
  nohup bash update_rsnum.sh tmp.list`printf "%03d" $i` $2 ${outname}`printf "%03d" $i`.txt >${outname}`printf "%03d" $i`.out &
  let i=$i+1
done
