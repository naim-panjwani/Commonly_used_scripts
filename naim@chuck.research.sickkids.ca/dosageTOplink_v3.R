...
block_lengths <- c(25, 25, 20, 20, 19, 
                   ...
                   10, 9, 8, 6, 7,
                   5, 6, 16)
block_starts <- c(14,1,1,1,1,
                  1,1,1,6,1,
                  1,1,2,3,3,
                  1,1,1,1,1,
                  2,2,1)
chr_block_avoid <- list(chr1=c(14), chr9=c(6))
...
#for (i in 1:23) { # for each chromosome
  assign("blocks_to_avoid", paste("chr_block_avoid$chr",i,sep=""))
  blocks_to_avoid <- eval(parse(text=blocks_to_avoid))
  blocks_to_convert <- block_starts[i]:block_lengths[i]
  if(!is.null(blocks_to_avoid) & length(which(blocks_to_convert %in% blocks_to_avoid))!=0) blocks_to_convert <- blocks_to_convert[-which(blocks_to_convert %in% blocks_to_avoid)]
  print(paste("chr",i))
  for (j in blocks_to_convert) { # for each block
